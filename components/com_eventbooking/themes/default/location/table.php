...
<form method="post" name="adminForm" id="adminForm" action="<?php echo Route::_('index.php?option=com_eventbooking&view=location&location_id=' . $this->location->id . '&Itemid=' . $this->Itemid) ; ?>">
	...
	if (count($this->items))
	{
		...
		{
			echo EventbookingHelperHtml::loadCommonLayout('common/events_table.php', ['items' => $this->items, 'config' => $this->config, 'Itemid' => $this->Itemid, 'nullDate' => $this->nullDate, 'ssl' => 0, 'viewLevels' => $this->viewLevels, 'categoryId' => 0, 'bootstrapHelper' => $this->bootstrapHelper]);
		}
		else
		{
			$this->categoryId =  0;
