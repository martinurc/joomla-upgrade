...
<form method="post" name="adminForm" id="adminForm" action="<?php echo Route::_('index.php?option=com_eventbooking&view=location&location_id=' . $this->location->id . '&Itemid=' . $this->Itemid) ; ?>">
	...
	if (count($this->items))
	{
		...
		{
			echo EventbookingHelperHtml::loadCommonLayout('common/events_default.php', ['events' => $this->items, 'config' => $this->config, 'Itemid' => $this->Itemid, 'nullDate' => $this->nullDate, 'param' => null, 'ssl' => 0, 'viewLevels' => $this->viewLevels, 'bootstrapHelper' => $this->bootstrapHelper]);
		}
		else
		{
			$this->categoryId =  0;
