...
if (!$event->is_multiple_date
	&& !$event->can_register
	&& ($event->registration_type != 3 || $event->published == 2)
	...
	if (isset($event->cannot_register_reason))
	{
		switch ($event->cannot_register_reason)
		{
			...
			case  'event_cancelled':
				...
				break;
			default:
				$languageKey = 'EB_' . strtoupper($event->cannot_register_reason);

				if (Factory::getLanguage()->hasKey($languageKey))
				{
					$msg = Text::_($languageKey);
				}
		}
	}
...
elseif (!empty($event->cannot_register_reason) && $event->cannot_register_reason === 'auto_register_child_event_is_full')
{
?>
	<div class="<?php echo $bootstrapHelper->getClassMapping('clearfix'); ?>">
		<p class="text-info eb-notice-message"><?php echo Text::_('EB_AUTO_REGISTER_CHILD_EVENT_IS_FULL'); ?></p>
	</div>
<?php
}
