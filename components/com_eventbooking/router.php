 ...
 * @since  2.8.1
 */
class EventbookingRouter extends RouterBase
{
	/**
	...
	public function build(&$query)
	{
		...
		if (empty($query['Itemid']))
		{
			...
			// Unset the Itemid variable in case it is set but empty
			unset($query['Itemid']);
		}
		else
		{
		...
		if ($menuItem && isset($query['view']) && $menuItem->query['view'] === $query['view'])
		{
			...
			if (isset($query['id'], $menuItem->query['id']) && $query['id'] == $menuItem->query['id'])
			{
				unset($query['id'], $query['catid'], $query['al']);
			}
		...
		// Do not add event alias to the registration link if current menu item is linked to event
		if ($menuItem && $menuItem->query['view'] == 'event'
			&& isset($menuItem->query['id']) && $menuItem->query['id'] == $eventId)
		{
			if (in_array($task, ['register.individual_registration', 'register.group_registration'])
				|| ($view === 'register' && in_array($layout, ['number_members', 'group_members', 'group_billing'])))
			{
				$eventId = 0;
				unset($queryArr['event_id']);
			}
		}

		// Language, pass from component association helper
		$language = $query['al'] ?? null;
	...
	public function parse(&$segments)
	{
		...
		if ($item && (!empty($vars['view']) && !empty($item->query['view']) && $vars['view'] == $item->query['view']))
		{
		...
		// If the active menu item is linked direct link to event and this is link to registration form, use event id from there
		if (!isset($vars['event_id'])
			&& $item
			&& isset($item->query['view'], $item->query['id'])
			&& $item->query['view'] === 'event'
			&& $item->query['id'] > 0)
		{
			$task   = $vars['task'] ?? '';
			$view   = $vars['view'] ?? '';
