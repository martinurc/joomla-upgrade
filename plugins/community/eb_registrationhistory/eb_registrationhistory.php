        ...
        if (count($rows)) {
			...
			<table width="100%" cellspacing="3" cellpadding="3" class="eb_registration_history">
				<tr>
					...
					<td class="sectiontableheader" width="55%">
                        <?php
                        echo Text::_('EB_EVENT'); ?>
					</td>
					<td class="sectiontableheader" align="center">
                        <?php
                        echo Text::_('EB_NUMBER_REGISTRANTS'); ?>
					</td>
					<td class="sectiontableheader">
                        <?php
                        echo Text::_('EB_REGISTRATION_DATE'); ?>
					</td>
					<td class="sectiontableheader" align="right">
                        <?php
                        echo Text::_('EB_AMOUNT') . ' (' . $symbol . ')'; ?>
					</td>
				</tr>
                <?php
            ...
            for ($i = 0, $n = count($rows); $i < $n; $i++) {
                ...
                echo $tab; ?>">
						...
						<td>
							<a href="<?php
                        echo Route::_('index.php?option=com_eventbooking&task=view_event&event_id=' . $row->event_id . '&Itemid=' . $itemId); ?>"
							   target="_blank"><?php
                            echo $row->event_title; ?></a>
						</td>
						<td align="center">
                            <?php
                        echo $row->number_registrants; ?>
						</td>
						<td>
                            <?php
                        echo HTMLHelper::_('date', $row->register_date, $dateFormat); ?>
						</td>
						<td align="right">
                            <?php
