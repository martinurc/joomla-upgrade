...
class AdminTools extends CMSPlugin implements SubscriberInterface, DatabaseAwareInterface
{
	...
	private static $featureClasses = [
		Feature\CustomErrorPageUncache::class,
        ...
        // Please note: notify BEFORE deleting the entries, or we might miss to notify
	...
	public function onPageCacheSetCaching(Event $event): void
	{
		$results = $this->runFeature('onPageCacheSetCaching');
		$results = array_filter($results, fn($x) => $x === false || $x === true);
