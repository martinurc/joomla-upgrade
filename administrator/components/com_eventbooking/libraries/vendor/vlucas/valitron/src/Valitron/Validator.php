...
class Validator
{
	...
	protected function validateCodiceFiscale($field, $value)
	{
		\JLoader::registerNamespace(
			'CodiceFiscale',
			JPATH_ADMINISTRATOR . '/components/com_eventbooking/libraries/vendor/davidepastore/codice-fiscale/src',
			false,
			true,
			'psr4'
		);
	...
	protected function validateUnique($field, $value)
	{
		...
		if ($rowField->is_core)
		{
			$query->clear()
				...
				->where('(a.published =  1 OR (a.published = 0 AND a.payment_method LIKE "os_offline%"))')
				->where($db->quoteName($rowField->name) . ' = ' . $db->quote($value));
		}
		else
		{
			$query->clear()
				...
				->where('(a.published =  1 OR (a.published = 0 AND a.payment_method LIKE "os_offline%"))')
				->where('b.field_id = ' . $rowField->id)
				->where('b.field_value = ' . $db->quote($value));
		}
	 ...
	 * @return bool
	 */
	protected function validateInTableColumn($field, $value, $params)
	{
		if (!is_string($value) || strlen($value) === 0)
		{
			return false;
