...
class RADPaymentOmnipay extends OmnipayPayment
{
	...
	protected function setPaymentSuccessUrl($id, $data = [])
	{
		...
		else
		{
			$this->paymentSuccessUrl = Route::_(
				'index.php?option=com_eventbooking&view=complete&registration_code=' . $row->registration_code . '&Itemid=' . $Itemid,
				false
			);
		}
	}
	...
	protected function beforeRequestSend($request, $row, $data)
	{
		...
		$request->setReturnUrl(
			$siteUrl . 'index.php?option=com_eventbooking&task=payment_confirm&id=' . $row->id . '&payment_method=' . $this->name . '&Itemid=' . $Itemid
		);
		$request->setNotifyUrl(
			$siteUrl . 'index.php?option=com_eventbooking&task=payment_confirm&id=' . $row->id . '&payment_method=' . $this->name . '&notify=1&Itemid=' . $Itemid
		);
