...
class EventbookingModelCommonRegistrant extends RADModelAdmin
{
	...
	protected function addNewRegistration($row, $input, $form)
	{
		...
		if (!$row->payment_method || ($row->published == 0 && strpos($row->payment_method, 'os_offline') === false))
		{
		...
		if ($row->total_amount == 0)
		{
			...
			else
			{
				$rate               = EventbookingHelper::callOverridableHelperMethod(
					'Registration',
					'getRegistrationRate',
					[$data['event_id'], $data['number_registrants']]
				);
				$feeCalculationTags = [
					...
					&& file_exists(JPATH_ROOT . '/components/com_eventbooking/fields.xml'))
				{
					...
					{
						$feeCalculationTags[strtoupper($customFieldName)] = $filterInput->clean(
							$param['value'],
							'float'
						);
					}
	...
	protected function updateRegistration($row, $input, $form)
	{
		...
		if ($eventChanged)
		{
			$app->triggerEvent('onRegistrantMovingToNewEvent', [$row, $input]);
		}
		...
		elseif ($row->published == 2 && $published != 2)
		{
			...
			$app->triggerEvent('onRegistrationCancelled', [$row, $published]);
		...
		{
			$app->triggerEvent('onAfterRegistrantStatusChanged', [$row, $published, $row->published]);
		}
	}
	...
	public function publish($cid, $state = 1)
	{
		...
		if (count($cid))
		{
			...
			foreach ($cid as $id)
			{
				...
				{
					$app->triggerEvent('onAfterRegistrantStatusChanged', [$row, $row->published, $state]);
				}
			}
		}
	...
	public function checkinRegistrant($id, $checkinAllGroupMembers = false, $checkEventDate = false)
	{
		...
		if ($row->published == 1)
		{
			...
			Factory::getApplication()->triggerEvent('onEBCheckinSuccess', [$row]);
	...
	public function batchCheckin($cid)
	{
		...
		{
			$row->load($id);
			$app->triggerEvent('onEBCheckinSuccess', [$row]);
			$app->triggerEvent('onAfterAdminCheckinRegistrant', [$row]);
		}

		$app->triggerEvent('onAfterAdminBatchCheckinRegistrants', [$cid]);
	}
