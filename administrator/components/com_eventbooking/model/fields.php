...
class EventbookingModelFields extends RADModelList
{
	...
	protected function buildQueryWhere(JDatabaseQuery $query)
	{
		...
		{
			$query->where(
				'(tbl.category_id = -1 OR tbl.id IN (SELECT field_id FROM #__eb_field_categories WHERE category_id=' . $state->filter_category_id . '))'
			);
		}
		...
		if ($state->filter_event_id)
		{
			...
			$query->where(
				'(tbl.event_id = -1 OR tbl.id IN (SELECT field_id FROM #__eb_field_events WHERE event_id = ' . $state->filter_event_id . ' OR event_id < 0))'
			)
