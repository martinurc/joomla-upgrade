...
class DatabaseTools extends Model
{
	...
	private function gcSessionJoomla3()
	{
		...
		/**
		 * When a handler other than 'database' is used, we need to remove database records from the `#__sessions`
		 * table.
		 *
		 * The reason is that JApplication::__construct() will call JApplication::_createSession() which in turn calls
		 * JApplication::checkSession(). This method creates a `#__sessions` record even if the storage handler is set
		 * to anything other than `database`.
		 *
		 * In this case, the storage handler's gc() method will not know about the `#__sessions` records, therefore the
		 * code above will do nothing. As a result, we have to do this manually.
		 */
		$jConfig = Factory::getConfig();

		if ($jConfig->get('session_handler', 'none') === 'database')
		{
			return;
		}

		$past = time() - $options['expire'];
		$db   = Factory::getDbo();
