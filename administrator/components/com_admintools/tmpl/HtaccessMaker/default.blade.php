...
<div class="akeeba-block--info">
	<p>
		<strong>
			@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_WILLTHISWORK')
		</strong>
	</p>
	<p>
		@if($this->isSupported == 0)
			@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_WILLTHISWORK_NO')
		@elseif($this->isSupported == 1)
			@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_WILLTHISWORK_YES')
		@else
			@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_WILLTHISWORK_MAYBE')
		@endif
	</p>
</div>
...
<div class="alert alert-info mb-3">
	<h3 class="alert-heading">
		@lang('COM_ADMINTOOLS_HTACCESSMAKER_LBL_PHPHANDLERS')
	</h3>
	<p>
		@lang('COM_ADMINTOOLS_HTACCESSMAKER_LBL_PHPHANDLERS_DESC')
	</p>
	<pre><?= $this->escape($this->handlers) ?></pre>
	<a href="@route('index.php?option=com_admintools&view=Htaccessmaker&task=addphphandler&'.$this->container->platform->getToken(true).'=1')" class="btn btn-success">@lang('COM_ADMINTOOLS_HTACCESSMAKER_LBL_PHPHANDLERS_APPLY')</a>
</div>
<?php endif;?>

<div class="akeeba-block--warning">
	<h3>@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_WARNING')</h3>

	<p>@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_WARNTEXT')</p>

	<p>@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_TUNETEXT')</p>
</div>
...
<form name="adminForm" id="adminForm" action="index.php" method="post" class="akeeba-form--horizontal">
	...
	<div class="akeeba-panel--primary">
		<header class="akeeba-block-header">
			<h3>@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_CUSTOM')</h3>
		</header>

		<div class="akeeba-form-group">
			<label for="custhead">@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_CUSTHEAD')</label>

			<textarea cols="80" rows="10" name="custhead"
					  id="custhead">{{{ $config->custhead }}}</textarea>
		</div>

		<div class="akeeba-form-group">
			<label for="custfoot">@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_CUSTFOOT')</label>

			<textarea cols="80" rows="10" name="custfoot"
					  id="custfoot">{{{ $config->custfoot }}}</textarea>
		</div>
	</div>
	<!-- ======================================================================= -->
	<div class="akeeba-panel--primary">
		...
		<div class="akeeba-form-group">
			<label for="forcegzip">@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_FORCEGZIP')</label>

			@jhtml('FEFHelp.select.booleanswitch', 'forcegzip', $config->forcegzip)
		</div>
		...
		<div class="akeeba-form-group">
			...
			@jhtml('FEFHelp.select.genericlist', \Akeeba\AdminTools\Admin\Helper\Select::getHstsOptions(), 'hstsheader', ['list.select' => $config->hstsheader])
		...
		<div class="akeeba-form-group">
			<label for="utf8charset">@lang('COM_ADMINTOOLS_LBL_HTACCESSMAKER_UTF8CHARSET')</label>

			<div>
				@jhtml('FEFHelp.select.booleanswitch', 'utf8charset', $config->utf8charset)
			</div>
		</div>
		...
		<div class="akeeba-form-group">
			...
			{{ \Akeeba\AdminTools\Admin\Helper\Select::etagtype('etagtype', ['class' => 'input-medium'], $config->etagtype) }}
		</div>
	...
	<div class="akeeba-panel--primary">
		...
		<div class="akeeba-form-group">
			...
			{{ \Akeeba\AdminTools\Admin\Helper\Select::symlinks('symlinks', ['class' => 'input-medium'], $config->symlinks) }}
		</div>
