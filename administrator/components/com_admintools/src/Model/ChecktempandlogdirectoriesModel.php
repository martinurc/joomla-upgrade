...
class ChecktempandlogdirectoriesModel extends BaseModel
{
	...
	private function checkTmpFolder()
	{
		...
		try
		{
			$folderExists = Folder::exists($tmpDir);
		}
		catch (\Exception $e)
		{
			$folderExists = false;
		...
		if ($folderExists && !is_writable($tmpDir))
		{
...
";
			...
			@file_put_contents($tmpDir . '/.htaccess', $contents);
	...
	private function checkLogFolder()
	{
		...
		try
		{
			$folderExists = Folder::exists($logDir);
		}
		catch (\Exception $e)
		{
			$folderExists = false;
		...
		if ($folderExists && is_writable($logDir))
		{
...
";
			...
			@file_put_contents($logDir . '/.htaccess', $contents);
	...
	private function saveConfigurationValue($key, $value)
	{
		...
		{
			opcache_invalidate($configurationFilePath, true);
		}
	}
	...
	private function chmod($dir)
	{
		try
		{
			$folderExists = Folder::exists($dir);
		}
		catch (\Exception $e)
		{
			$folderExists = false;
