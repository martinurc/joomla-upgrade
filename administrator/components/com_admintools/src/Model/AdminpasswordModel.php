...
class AdminpasswordModel extends BaseModel
{
	...
	{
		$cryptpw      = $this->apacheEncryptPassword($this->getState('hashtype', 'apr1'));
...
## Password hashing algorithm used:   {$this->getState('hashtype')}
##
...
HTACCESS;
	...
	public function unprotect(): bool
	{
		...
		return @unlink($htaccessPath) && @unlink($htpasswdPath);
	}
	...
	protected function populateState()
	{
		...
		$hashtype = $app->getUserStateFromRequest('com_admintools.adminpassword.hashtype', 'hashtype', 'apr1', 'cmd');
		$this->setState('hashtype', $hashtype);
	}
	...
	protected function apacheEncryptPassword(?string $hashType = 'apr1'): string
	{
		...
		// Set the hash type into the session.
		$this->setState('hashtype', $hashType);
		...
		if ($hashType === 'apr1')
		{
			...
			return $this->apr1_hash($this->getState('password'), $salt, 1000);
		}
		...
		{
			$sha1 = $hasHashSha1
				? hash('sha1', $this->getState('password'), true)
				: sha1($this->getState('password'), true);
		...
		if ($hashType === 'crypt')
		{
			...
			return crypt($this->getState('password'), $salt);
		}
	...
	protected function apr1_hash($password, $salt, $iterations)
	{
		...
		$bin  = hash('md5', $password . $salt . $password, true);
